...
class Transformer(torch.nn.Module, ABC):
    ...
    def __init__(self,  # network structure related
        ...
        if self.use_guided_attn_loss:
            self.attn_criterion = GuidedMultiHeadAttentionLoss(sigma=guided_attn_loss_sigma, alpha=guided_attn_loss_lambda)
        self.load_state_dict(torch.load(os.path.join("Models", "TransformerTTS_Blizzard_Finetune", "best.pt"), map_location='cpu')["model"])
...
class MelGANGenerator(torch.nn.Module):
    ...
    def __init__(self, in_channels=80, out_channels=1, kernel_size=7, channels=512, bias=True,
        ...
        self.load_state_dict(torch.load(os.path.join("Models", "MelGAN_Blizzard", "best.pt"), map_location='cpu')["generator"])
...
class SpanishBlizzard_TransformerTTSInference(torch.nn.Module):
    ...
    def __init__(self, device="cpu", speaker_embedding=None):
        ...
        self.device = device
        self.text2phone = TextFrontend(language="es", use_codeswitching=True)
    ...
    def forward(self, text, view=False, jupyter=True):
        ...
        if view:
            ...
            ax[0].set_title(self.text2phone.string_to_tensor(text, return_string=True))
